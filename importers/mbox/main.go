// Command mbox-importer drives a one-shot import of a finished mbox
// archive into glovebox. Spec 09 design + glovebox-7ey for the bead.
//
// Flow:
...
//      mboxImporter.Import.
//   4. SIGTERM / SIGINT cancel the context; mboxImporter.Import
//      writes manifest.status=interrupted + checkpoint so the next
//      run resumes from the same byte offset.
package main
...
func runCtx(ctx context.Context, args []string) int {
	...
	source := fs.String("source", "", "path to the mbox file (required)")
	...
	sourceName := fs.String("source-name", "mbox", "value for ingest metadata source field")
