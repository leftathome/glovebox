// Command apple-importer drives a one-shot import of a finalized Apple-export
// staged archive directory into glovebox. The archive is a directory produced
// by the ingest pipeline (e.g. archives/<id>/), not a single flat file.
//
// Flow:
//  1. Parse CLI flags (or read GLOVEBOX_* env vars).
//  2. connector.NewFramework() bootstraps logger / config / rule matcher /
//     staging backend (HTTP if GLOVEBOX_INGEST_URL is set, filesystem
//     otherwise) / metrics / health server. The config's data_subject_default
//     / audience_default are auto-wired into the backend via
//     SetConfigDataSubject / SetConfigAudience.
//  3. importer.RunOneShot() orchestrates the spec 09 §3.1 sequence: ensure
//     survey, decide resume action, load filter, hand off to
//     appleImporter.Import.
//  4. SIGTERM / SIGINT cancel the context.
package main
...
import (
	...
	// each subpackage's init(). The skeleton apple importer passthrough-stages
	// each tree file; the enricher set mirrors the other archive importers so
	// staged content is enriched consistently. Removing an import simply
	// removes that enricher from this importer's pipeline.
...
// run is the testable entry point: returns the exit code rather than calling
// os.Exit directly so integration tests can drive it without
// process-isolation.
func run(args []string) int {
	fs := flag.NewFlagSet("apple-importer", flag.ContinueOnError)
	...
	sourceName := fs.String("source-name", "apple", "value for ingest metadata source field")
	...
	if *ingestURL != "" {
		// connector.selectBackend reads GLOVEBOX_INGEST_URL from env. Set it
		// from the flag so a single override path produces the HTTP backend
		// choice.
	...
	fw, err := connector.NewFramework(connector.Options{
		...
		// No Connector / Setup -- one-shot importer pattern; the framework's
		// poll loop is not used.
	})
	if err != nil {
	...
	if err := importer.RunOneShot(ctx, fw, imp, cfg); err != nil {
		// Cancellation is a clean exit -- we want the next run to resume; a
		// non-zero exit would imply an operator-actionable failure.
		// Distinguish via ctx.Err().
		if ctx.Err() != nil {
...
// signalContext returns a context that is cancelled on SIGTERM / SIGINT.
