...
2. **Invisible character stripping** -- removes every Unicode **Default_Ignorable_Code_Point** plus the remaining `Cf` format characters. The default-ignorable set is *derived* at startup from Go's unicode tables using the formula in UCD `DerivedCoreProperties.txt` (`internal/engine/invisible.go`: `DefaultIgnorable`), not hand-listed, and a test pins it code point for code point against the published table (4174 code points in Unicode 17.0). It covers the zero-width space/joiners, BOM and word joiner, the soft hyphen (U+00AD), the bidi marks (U+061C, U+200E, U+200F), the explicit bidi embeddings, overrides and isolates (U+202A-U+202E, U+2066-U+2069), the invisible math operators (U+2061-U+2064), the deprecated format controls (U+206A-U+206F), the Hangul fillers, the combining grapheme joiner, the Mongolian vowel separator and free variation selectors, the Khmer inherent vowels, variation selectors, the shorthand and musical format controls, the reserved default-ignorable code points, and the **Unicode Tags block** (U+E0000-U+E007F). The Tags block is the invisible-ASCII channel: a payload written there renders as nothing but is read back verbatim by a model, so it is additionally *decoded* and fed to the ordinary rules (see below) and flagged by `invisible_smuggling`. Stripping runs again after HTML entity decoding, on every unescaped or reordered view, and on every decoded base64/hex/percent run, because a character reference (`&#8203;`, `&shy;`, `&zwj;`) or an escape only becomes an invisible once decoded. `suspicious_encoding` counts `IsSuspiciousInvisible` as "zero-width characters": Default_Ignorable_Code_Point minus the variation selectors (U+FE0F follows ordinary emoji; ideographic variation sequences spell CJK names), minus the soft hyphen (ordinary CMS output, and what `&shy;` decodes to), and minus the Tags block (owned by `invisible_smuggling`). All of those are still stripped. An identifier canonicaliser should drop the whole of Default_Ignorable_Code_Point.
...
| `suspicious_encoding` | custom_detector | 0.7 | Base64 blocks in plain text, zero-width chars (`IsSuspiciousInvisible`: Default_Ignorable_Code_Point minus variation selectors, the soft hyphen and the Tags block), bidi controls, excessive unicode escapes |
...
- **`language_detection`** — uses `lingua-go` to classify content language; fires when primary language is not English. Requires minimum content length (20+ characters) for reliable detection. Returns detected language in signal details. **It classifies the item's *prose*, not its bytes.** ASCII armour (PEM/PGP `-----BEGIN ... -----END` blocks), `data:` URIs and unbroken base64/base64url/hex token runs are excised first (`engine.StripStructured`), and if fewer than 20 non-whitespace characters of prose survive, no language is named. The booster's justification is that the matcher patterns are English-only, so non-English *writing* under-reads; armoured data is not writing in any language, and asked about it anyway lingua answers confidently and wrongly — an inline PNG classified as Dutch at confidence 1.00 boosted `suspicious_encoding` from 0.70 to 1.05 and quarantined every inline image and every PGP-signed message. This narrowing applies to this detector's input only: matchers, `encoding_anomaly` and the decode-then-scan buffers still see the whole item, armour included.
