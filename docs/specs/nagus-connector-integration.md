...
|---|---|---|
| **eBay Browse** (HDD used/refurb listings; later durables broadly) | Yes (title/description) | **glovebox -- NEW `cmd/connector-ebay`** |
| **Craigslist `reo`** (land listings) | Yes (body) | **glovebox -- existing `connector-rss`** + config (add feed + routing rule) |
...
1. **`cmd/connector-ebay`** (new): eBay Browse API (OAuth client-credentials, ~5k/day).
...
2. **Craigslist via `connector-rss`** (config, not new code): add the `reo` search feed
...
3. **Handoff contract (glovebox -> nagus):** define where the **extract/tokenize** step
