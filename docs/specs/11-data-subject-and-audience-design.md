...
identifier ("Bee's guardians", "Bee's siblings", "Bee's caregivers"). When
...
roles are inherently subject-relative. Resolution ("is Steve one of Bee's
guardians? is the math-tutor agent in Bee's caregivers?") happens at the
...
2. **No way to address delegated supervisors.** A real audience for an
   item like "Bee's math homework" can include not just her guardians but
