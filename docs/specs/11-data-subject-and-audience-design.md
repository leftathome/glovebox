...
- Medical-care role tokens (`spouse`, `medical_providers`, etc.) and
  ...
  (e.g., pediatrician comms about Child 1) with `[guardians]` or
...
Example: a pediatrician's note about Child 1 at age 12 is stamped
`audience: ["guardians"]` because Child 1 was not the intended recipient at the
time. When Child 1 turns 18, the agent serving her may decide that all
...
for Child 1's assignments, and the rule matcher fills in `data_subject` and
`audience`.
